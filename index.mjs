...
/** Send reference payload to GHL onboarding webhook (for mapping/setup). Operator only. */
app.post("/api/onboarding/:id/trigger-webhook/onboarding", requireOperator, async (req, res) => {
  const record = await db.getOnboarding(req.params.id);
  if (!record) return res.status(404).json({ error: "Not found" });
  const webhookUrl = process.env.GHL_ONBOARDING_WEBHOOK_URL;
  if (!webhookUrl) return res.status(400).json({ error: "GHL onboarding webhook URL not configured" });
  const merged = record.payload_json || {};
  const now = new Date().toISOString();
  const webhookPayload = {
    ...merged,
    client_email: record.client_email ?? null,
    meta: {
      onboarding_id: req.params.id,
      submitted_at: now,
      info_complete_at: computeInfoComplete(merged) ? now : null,
      launch_clock_start_at: shouldStartLaunchClock(merged) ? now : null,
    },
  };
  try {
    ...
    console.error("[Clara server] GHL onboarding webhook trigger error:", err);
    return res.status(502).json({ error: "Webhook request failed" });
  }
  res.json({ ok: true });
});

/** Send reference payload to GHL agreement webhook (for mapping/setup). Operator only. */
app.post("/api/onboarding/:id/trigger-webhook/agreement", requireOperator, async (req, res) => {
  const record = await db.getOnboarding(req.params.id);
  if (!record) return res.status(404).json({ error: "Not found" });
  const webhookUrl = process.env.GHL_AGREEMENT_WEBHOOK_URL;
  if (!webhookUrl) return res.status(400).json({ error: "GHL agreement webhook URL not configured" });
  const merged = record.payload_json || {};
  const agreementPayload = {
    onboarding_id: req.params.id,
    client_email: record.client_email ?? null,
  ...
  try {
    ...
    console.error("[Clara server] GHL agreement webhook trigger error:", err);
    return res.status(502).json({ error: "Webhook request failed" });
  }
  res.json({ ok: true });
});
